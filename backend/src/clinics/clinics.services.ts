...
import { Clinic } from './clinics.entity'; // Добавляем импорт Clinic
...
export class ClinicsService {
    ...
    async create(clinic: Omit<Clinic, 'id' | 'createdAt' | 'updatedAt'>): Promise<Clinic> {
        const query = `
            INSERT INTO clinics (name, address, phone, email, work_hour_start, work_hour_end)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *
        `;
        const values = [
    ...
    async update(id: number, clinic: Partial<Clinic>): Promise<any> {
        const query = `
            UPDATE clinics
            SET name = $1, address = $2, phone = $3, email=$4, work_hour_start = $5, work_hour_end = $6
            WHERE id = $7
            RETURNING *
        `;
        const values = [
